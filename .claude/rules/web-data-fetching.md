...
- `beginOptimisticQueryUpdate` — coordinate overlapping writes to one query in submission order. Every optimistic writer, including a create that only invalidates, must participate. Replay functions must be synchronous, deterministic, immutable, and only update that query. The group rebases its rollback state on successful refetches and authoritative cache replacements, then reapplies the remaining projections. Infinite-query page loads retain their server baselines separately from optimistic existing pages, so paging remains available during mutations. Refetch after the last `settle` returns true; repeated settlement and handles from removed queries return false without changing data. Initial apply errors restore the prior cache and release their entry; a replay error discards that projection while the real mutation remains pending. Reference: currency transaction create/edit/delete in `use-currencies.ts` and [the design notes](../../docs/design/testing-and-tooling.md#rollback-and-overlapping-mutations). This prevents failed mutations from erasing other edits, newly loaded pages, or newer server data.
