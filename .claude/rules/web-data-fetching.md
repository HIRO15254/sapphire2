...
- `beginOptimisticQueryUpdate` — coordinate overlapping writes to one query in submission order. Every writer, including a create that only invalidates, must participate; replay functions must be deterministic and only update that query. Refetch after the last `settle` returns true. Reference: currency transaction create/edit/delete in `use-currencies.ts`. This prevents one failed edit or an earlier create refetch from erasing another pending update.
