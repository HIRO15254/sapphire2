...
- Triage every NoCoverage mutant into one of three outcomes before touching it. **Test** when it sits in validation, ownership WHEREs, arithmetic, state transitions, date handling, sorting, error-code selection, batch composition, or the content of a write (what row is inserted / updated), or when the enclosing procedure's success path has never been called — field pass-through in an `update` gets one `it()` per procedure passing every optional field, never one per field. **Directive** (`// Stryker disable next-line <Mutator>: equivalent: <why>`) when the branch is provably unreachable from its only call sites — an index guard on an element taken from `findIndex !== -1` or from a dense spread-and-sort array, a `default` on a TypeScript-exhaustive union, a condition the caller already threw on. **Leave it** when the only test would pin the mock rather than the behavior: a "row vanished between two selects" guard after a write (reaching it means hand-counting the mock's `select()` order), a procedure that returns a standalone-tested helper's result unchanged, a thin browser-API wrapper whose test restates its stub. Leftovers are listed with their reason in `docs/design/testing-and-tooling.md` so the next sweep does not re-litigate them.
...
- A component or page spec (`apps/web/src/**/{components,pages}/**/*.test.tsx`, `apps/web/src/__tests__/*.test.tsx`) that calls `vi.mock("@tanstack/react-query", …)` fails `check:rules`: mock the component's own `use-*` hook instead. Hook specs are not in scope — they use a real `QueryClient` from `createTestQueryClient`.
