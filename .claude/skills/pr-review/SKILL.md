...
Launch all applicable finders in parallel. Give each: the range and its file list, the PR title/body, the agent assumptions, the candidate schema below, and the **What NOT to flag** list at the end of this file. Every candidate must name a concrete failure scenario; a candidate without one is discarded before validation. A real bug a finder notices outside its remit is still returned as a candidate with `rule_citation: none`, never as a side remark: validators decide, finders do not self-censor. In particular a finder never drops a candidate because it judged the input unreachable, the callers safe, or the scenario undemonstrated — that judgement is the validator's verdict, made with tests and callers open; the finder emits the candidate with confidence 20–40 and states the doubt in `failure_scenario`. The What NOT to flag list is applied by the orchestrator in Step 3, not by finders. A finder report that mentions a considered-and-dropped candidate is a defect of the report.
...
- `confidence` measures one thing: how sure you are that the described input or state produces the described behaviour. It is not discounted because the remedy is a product or design decision, because a document describes the behaviour as intended (a documented behaviour a user would not expect is still a finding), because the tier is debatable, or because another finder rated it low. A CONFIRMED verdict with a reproduced or quoted mechanism is 85 or above; put the doubt about remedy or tier in `evidence`, not in the number.
...
- Tier is decided here, not by the finder: **important** = wrong behaviour a user or an MCP client will hit, data loss or corruption (including a partial, truncated, or defaulted result accepted and shown as a success), a security gap, a migration that can stop production, or a missing test for a bug this PR fixes; **nit** = real but cosmetic or edge-case, or a rule violation with no observed failure; **pre-existing** = real but not introduced by this range.
