...
```bash
gh issue edit {ISSUE_NUMBER} --remove-label "wf:needs-spec" --add-label "wf:spec-review"
```
...
gh issue edit {ISSUE_NUMBER} --remove-label "wf:needs-spec" --add-label "wf:blocked"
```
