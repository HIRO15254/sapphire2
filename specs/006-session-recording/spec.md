...
**Created**: 2026-03-29
**Status**: Draft (UX Feedback Revision 2026-03-30)
...
ユーザーがキャッシュゲームに参加する際、ゲーム設定を選択し初回バイイン額を入力してセッションを開始する。ゲーム選択時にはmaxBuyInが自動入力され、通貨も自動紐づけされる。セッション中はスタック変動を記録し、スタック記録時にオールイン情報（ポットサイズ・試行回数・勝率・勝利数）やアドオン（追加バイイン）をボトムシートで追加できる。追加された記録はバッジとして表示され、タップで編集・削除が可能。セッション終了時にキャッシュアウト額を記録してP&Lが自動算出される。ライブセッション中のすべての画面は1画面に収まる設計とする。
...
1. **Given** ユーザーがキャッシュゲーム設定と初回バイイン額を入力した状態, **When** セッション開始を実行する, **Then** セッションが「進行中」状態で作成され、開始時刻とバイインイベントが記録される
...
4. **Given** 進行中のキャッシュゲームセッションがある状態, **When** スタック変動を記録する, **Then** 記録時刻とスタック額がイベント履歴に追加される
...
7. **Given** 進行中のキャッシュゲームセッションがある状態, **When** スタック記録画面で「Add Addon」ボタンを押す, **Then** ボトムシートが開き、アドオン額を入力して追加バイインを記録できる。アドオンは合計コストに反映される
8. **Given** 進行中のキャッシュゲームセッションがある状態, **When** セッションを終了しキャッシュアウト額を入力する, **Then** セッションが「完了」状態になり、P&Lが自動計算される
...
1. **Given** 完了済みのセッションがある状態, **When** セッションを再始動する, **Then** セッションが「進行中」状態に戻り、追加のイベントを記録できるようになる
...
4. **Given** 進行中のセッションがない状態, **When** ライブセッション一覧で完了済みセッションの再始動を選択する, **Then** 選択したセッションが進行中に戻る
...
- **FR-004**: ユーザーはキャッシュゲームセッション開始時に初回バイイン額を入力しなければならない。リングゲーム設定のmaxBuyInが初期値として自動入力される。2回目以降のバイイン（追加チップ購入）はスタック記録のアドオンとして記録する
...
- **FR-015**: ユーザーはキャッシュゲームのスタック記録時に、前回記録以降に発生したアドオン（追加バイイン）をボトムシート経由で記録できなければならない。追加済みのアドオンはバッジとして表示され、タップで編集・削除が可能
- **FR-016**: ユーザーはトーナメントのスタック記録時に、前回記録以降に発生したリバイ・アドオンを付随情報として記録できなければならない
- **FR-006**: ユーザーは完了済みのセッションを再始動（reopen）して「進行中」に戻し、追加のイベントを記録できなければならない。再度完了時にP&L・pokerSession・currencyTransactionが再計算される
...
- **FR-019**: ボトムナビゲーションはライブセッションの状態に応じて動的に変化しなければならない。進行中のセッションがある場合：中央の強調ボタンは現在のセッション画面へ遷移し、他のナビはイベント履歴・同卓者等の画面。セッションがない場合：中央ボタンで新規開始または再始動が可能、他のナビは通常のまま
...
- **SessionEvent（セッションイベント）**: セッション中の各操作を表すイベントエンティティ。キャッシュゲーム・トーナメント両方のセッションで共有する。イベント種別（バイイン、スタック記録、プレイヤー参加/退席、トーナメント状況更新など）、タイムスタンプ、関連データ（金額、プレイヤーID、残り人数など）を持つ。スタック記録イベントには付随情報として、オールイン情報（ポットサイズ・試行回数・勝率・勝利数）、アドオン（追加バイイン）、リバイを紐づけることができる。中断・再開イベントは廃止。
...
- 初回バイインはセッション開始時の必須入力。2回目以降のチップ追加はスタック記録のアドオンとして記録する
...
- Q: 2回目以降のバイインは？ → A: スタック記録のアドオンとして記録。スタンドアロンのバイインイベントは廃止
...
- Q: ボトムナビの挙動は？ → A: セッション進行中は中央強調ボタンでセッション画面へ遷移、他ナビはイベント履歴等。それ以外は中央ボタンで新規開始/再始動
