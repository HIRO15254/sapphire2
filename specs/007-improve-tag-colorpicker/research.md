...
- **Decision**: スウォッチサイズ `w-8 h-8`（32px）、`p-1`（パディング）でボタン全体のタッチエリアを 40px 以上確保。Constitution VI の 44px 要件に対し、`p-2` に拡大または `min-w-[44px] min-h-[44px]` を設定することで対応
- **Rationale**: スウォッチは小さすぎると押しにくい。44px タッチターゲットで Constitution VI を満たす
