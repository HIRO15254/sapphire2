...
- **クライアントは名前なしで登録してくることがあります。** RFC 7591 で `client_name` は任意であり、Claude のコネクタは送ってきません。Worker は better-auth が行を書く前に空の名前を補うため、`NOT NULL` 制約で登録が失敗することはなく、同意画面にはリダイレクト先ホストとともに *Unknown application* と表示されます。
