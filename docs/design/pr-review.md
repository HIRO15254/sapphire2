...
**State comment** — one PR comment starting with `<!-- pre-merge-review:state {"rounds":N,"lastSha":"…"} -->`, updated after every successful round (`renderStateComment`). It is the only memory the loop has; deleting it resets the PR to round 1. `parseReviewState` rejects malformed payloads instead of guessing, so a damaged comment also resets rather than misfiring. A round is recorded only when the action produced an execution log: `claude-code-action` exits with outcome `success` and no log on a PR whose copy of the workflow file differs from the default branch's (its OIDC validation), so a PR that edits `pre-merge-review.yml` is never reviewed until it merges, and that non-run must not spend one of the two rounds (it did on #596 before this guard).
