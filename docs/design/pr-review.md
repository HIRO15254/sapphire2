...
**State comment** — one PR comment starting with `<!-- pre-merge-review:state {"rounds":N,"lastSha":"…"} -->`, updated after every successful round (`renderStateComment`). It is the only memory the loop has; deleting it resets the PR to round 1. `parseReviewState` rejects malformed payloads instead of guessing, so a damaged comment also resets rather than misfiring. A round is recorded only when the run actually reported **and the report reached the PR**: `parseReviewResult` reads the action's execution log and looks for the summary's machine-readable trailer, `Confirm the summary reached the tracking comment` then re-reads that comment from the API and requires `hasPublishedSummary` (the `### レビュー結果` heading the skill mandates, or the trailer), and `Record the round` requires both. Two ways of not reviewing hide behind an outcome of `success`, and neither may spend one of the two rounds. `claude-code-action` exits `success` with no execution log at all on a PR whose copy of the workflow file differs from the default branch's (its OIDC validation), so a PR that edits `pre-merge-review.yml` is never reviewed until it merges (it spent a round on #596 before that guard); this one is reported in the step summary only, because every run on such a PR would otherwise repeat the same PR comment. And a run can produce a log, exit `success`, and still never post a summary, leaving the tracking comment an empty checklist that reads exactly like "the review found nothing" (#604 round 2 — SA2-231). The trailer check alone did not close that: the action fetches the tracking comment and adds its job link, never filling the body from the final message, so a reviewer that skips `update_claude_comment` publishes nothing while the trailer still says the round finished — which is how #620 round 2 spent a round on a summary nobody could read. The skill now states plainly that the summary goes in that comment, and the tracking-comment check is what makes the round conditional on it. Anything that ends without a trailer — that case, a turn-cap cutoff, or an internal error — gets the `<!-- pre-merge-review:truncated -->` notice on the PR naming the result `subtype`, and the notice is deleted once a round completes so a stale warning does not mislead in the other direction.
...
## Run 7: exhaustive validator assignment, removed-behaviour angle, two new PRs (2026-09-04)

Changes measured: every deduped candidate must be assigned to a validator before any launches (36-candidate capacity), F2 gained the **removed behaviour** angle (grep for every path that relied on a deleted hook, fallback, guard, redirect, or route), and the summary hard limits forbid findings outside the table. Cases: the two weak PRs from run 6, #590 as a stability control, and three PRs new to this skill — #572 (AI extraction truncation), #588 (EV null gate), #576 (live-session field editing, 20 files, 6 known defects, ground truth built by a Sonnet subagent from the audit data).

| Case | Files | Known real | Found | Important | Nits | Cost | Minutes |
|---|---|---|---|---|---|---|---|
| #568 filter presets | 36 | 4 | 2 | 3 | 6 | $25.99 | 19 |
| #592 OAuth login continuation | 5 | 2 | 1 | 2 | 0 | $7.31 | 17 |
| #590 passkeys | 38 | 3 | 2 | 2 | 9 | $17.15 | 16 |
| #572 AI extraction truncation | 11 | 1 | 1 | 0 | 2 | $7.88 | 13 |
| #588 EV null gate | 3 | 2 | 1 | 2 | 1 | $10.45 | 13 |
| #576 live-session field editing | 20 | 6 | 3 | 4 | 2 | $22.36 | 21 |

- **Run-to-run variance is now the dominant error term.** #568 found 2 of 4 in both runs 6 and 7 but a different pair: run 7 surfaced the statistics default preset that never fires (`isUrlEmpty` is always false in the browser — the miss that motivated the run 6 analysis) and lost the `Object.keys` count; the union of the two runs is 3 of 4. #590 dropped from 3 to 2: the silent passkey re-registration found in run 6 through the entity table did not recur, so the entity table was written in one run and not the other. On the two known items that motivated this run: the UNIQUE-regex candidate was again not validated, and reading the finder transcript shows why — the F1 shard found it and then wrote "turned out to be unreachable given how `create`/`update` actually invoke them, so I dropped them", which is the validator's decision made by the finder; finders are now forbidden to drop a candidate on reachability and must return it at confidence 20–40 with the doubt stated (measured in run 8). The preview auto-login hole in #592 was missed again: F2 enumerated the sign-in and sign-up forms as the paths relying on the removed hook and stopped, so the removed-behaviour list must now come from a grep the finder quotes, and both the entity table and the grep are mandatory sections of F2's report that the orchestrator checks (measured in run 9).
- **New PRs.** #572: the known defect (a truncated response whose partial JSON still parses is returned as a success) was found, but tiered `nit`, so the verdict was `approve`; the tier definition now names a partial or truncated result shown as success as data corruption. #588: the population-claim contradiction was found; the phantom-zero gate was not, but a new important finding is adjacent to it — `session.list`'s summary keeps the old fallback gate, so the same `totalEvDiff` field means different things in two MCP tools. #576: 3 of 6 (duration stretch on date edit, overnight end-time day, stale seed overwriting Events-section edits) plus a new important (a running session's `sessionDate` left stale after a start-date edit) and two nits; the misses are the local-getter date seed, the `bountyPrizes ?? 0` normalisation, and the required-only-for-live fields.
- **Cost is stable per file count**: $7–10 for 3–11 files, $17–26 for 20–38 files. #592's second important finding — a tRPC request carrying a live `oidc_login_prompt` cookie with a session due for refresh throws from `authorizeMCPOAuth` inside `createContextFactory` — was CONFIRMED at 88 from library source and is a candidate for a human check.

## Runs 8 and 9: finders may not self-drop, F2's table and grep are mandatory sections (2026-09-04)

Run 8 (#568 only) measured one change: a finder never drops a candidate on its own reachability judgement — it returns it at confidence 20–40 with the doubt stated, and the What NOT to flag list is applied by the orchestrator, not by finders. Run 9 (#592, #590, #576, #588) measured: F2's report must begin with the entity table and the removed-behaviour grep (command quoted, path list), and the orchestrator re-launches an F2 that lacks either. Both runs were first killed by the account usage limit (every result was the limit message at $1.5–3.9) and re-run after the reset.

| Case | Run | Known real | Found | Important | Nits | Cost | Minutes |
|---|---|---|---|---|---|---|---|
| #568 filter presets | 8 | 4 | 2 | 3 | 4 (+1 unverified) | $27.21 | 20 |
| #592 OAuth login continuation | 9 | 2 | 1 | 1 | 1 | $10.25 | 17 |
| #590 passkeys | 9 | 3 | 3 | 4 | 5 | $22.15 | 30 |
| #576 live-session field editing | 9 | 6 | 3 | 3 | 2 | $20.57 | 18 |
| #588 EV null gate | 9 | 2 | 1 | 0 | 2 (+1 pre-existing) | $9.71 | 12 |

- **Both targeted misses are fixed at the stage they were lost.** #568: the UNIQUE-regex breadth, self-dropped by the F1 finder in runs 5–7, reached a validator and is in the table (`db-errors.ts:38`, nit). #590: the silent passkey re-registration recurred once the entity table was a required section (run 7 had lost it), so #590 is 3 of 3 again. The union over runs 6–8 on #568 is now 4 of 4 while each run still yields 2 — the `Object.keys` count and the default-preset miss alternate between runs.
- **The #592 preview auto-login miss moved to the last stage.** The F2 report's grep section listed the preview auto-login as a path that lost the continuation (its `authClient.signIn\n.email(...)` call is split across lines, so the naive `signIn\.` pattern misses the file and only the broader `authClient\.` clause found it), a Sonnet validator CONFIRMED it at 85, and the orchestrator then folded it into the sign-up finding as "the same mechanism" and it vanished from the table. Dedupe is now limited to the same file and line range, and a coverage check before the summary lists every CONFIRMED verdict ≥ 80 against the table rows (measured in run 10).
