...
| Inline edit fields without a border until focused, committed on blur | `CRYST_INLINE_FIELD`; `selected-player-panel/`, `sheets/scan-seats-sheet/scan-review-row.tsx`, `sheets/mix-editor-sheet/mix-group-card.tsx` | Specified by the design file for editing names in place inside rows. The player panel has no Save action, so edits commit on blur (P3a, SA2-222). The mix group name uses the same field but is kept by the editor's ✓ like the rest of the sheet (P4b, SA2-224). |
...
| Blinds tab rows: the running level tinted with `primary` at 10%, breaks labeled in `warning`, and a pill-shaped Games button tinted with `info` once the level has its own games | `sheets/session-sheet/session-blinds-tab.tsx` | Specified by the design file's Blinds tab. The system reserves `primary` for actions and selection and has no pill button. The Games button keeps a solid border where the design draws it dashed, like the system's outline Button (P4b, SA2-224). |
