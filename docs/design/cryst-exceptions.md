...
| Blinds tab rows: the running level tinted with `primary` at 10%, breaks labeled in `warning`, and a pill-shaped Games button tinted with `info` once the level has its own games | `sheets/session-sheet/session-blinds-tab.tsx` | Specified by the design file's Blinds tab. The system reserves `primary` for actions and selection and has no pill button. The Games button keeps a solid border where the design draws it dashed, like the system's outline Button. A level with games takes each group's stakes on its row through the same inline caption fields the design gives the third blind and ante (P4b, SA2-224). |
