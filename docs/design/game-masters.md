...
**One game group per group, enforced by the server (SA2-224).** The schemas cannot see the masters, so [`packages/api/src/services/game-structure.ts`](../../packages/api/src/services/game-structure.ts) checks it on every write: the legacy `mix` sentinel inside `reconcileCashRuleSelection` (a named mix is already held to its canonical per-group buckets by `assertNamedMixComposition`), and every level-games path — `tournament.createWithLevels` / `updateWithLevels`, `blindLevel.create` / `update`, `session.create` / `update` for tournaments and `liveTournamentSession.updateSnapshot`. Each label resolves to the caller's variant by normalized label; a label that matches no variant is not checked (level labels are free text), and a group whose resolved variants span two `groupId`s is rejected with `BAD_REQUEST`.

A group whose members all sit inside one **stored** group is exempt: the record being replaced, the ring game passed as `ringGameId`, or the tournament master passed as `tournamentId`. A variant can change group after it was used (`gameVariant.update` accepts `groupId`), which turns stored groups mixed; the exemption keeps echoes, blind edits and game removals working on that data while still refusing to add a game to such a group.
