...
- `usePreviewAutoLogin` — only active under `VITE_PREVIEW_AUTO_LOGIN=true`, but that is exactly the preview environment used to verify this flow by hand. Alone among the three it signs in with **no user action**, so its resume is claimed once per authorize URL in `sessionStorage`: the in-component `attempted` ref only spans one page load, and `window.location.assign` starts a new one. Without that claim, an authorize endpoint that bounces back to `/login` — which a browser blocking the cross-site `SameSite=None` session cookie will cause on the preview deployment — would auto-sign-in and re-assign forever. The claim bounds **that leg only**, and only where it can be stored. A preview session whose cookie never lands is still unusable, because `/statistics` finds no session and the root route sends it back to `/login`; that inner bounce predates the continuation work and is not what the claim addresses. And a browser that lets the read through but throws on the write (quota exhausted, old Safari private mode) takes the same `catch` and re-assigns every time — the fallback deliberately keeps the pre-claim behavior rather than stranding a working flow, so nothing is lost, but nothing is bounded there either.

That list is not a promise anyone has to remember: [`scripts/check-rules.ts`](../../scripts/check-rules.ts) fails when any non-test file under `apps/web/src/**` calls `authClient.signIn` / `authClient.signUp` without importing `login-continuation` — deliberately not scoped to `features/auth/`, because auth hooks already live outside it (`shared/hooks/use-sign-out.ts`) and the invariant is about entry points, not directories. It checks that the import is present, so a partial regression (one branch still navigating straight to `/statistics`) still needs a reviewer. The invariant was written as prose first and broken twice before the check existed, which is exactly the case `AGENTS.md` reserves a `check-rules` entry for.
