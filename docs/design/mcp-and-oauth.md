...
- `usePreviewAutoLogin` — only active under `VITE_PREVIEW_AUTO_LOGIN=true`, but that is exactly the preview environment used to verify this flow by hand. Alone among the three it signs in with **no user action**, so its resume is claimed once per authorize URL in `sessionStorage`: the in-component `attempted` ref only spans one page load, and `window.location.assign` starts a new one. Without that claim, an authorize endpoint that bounces back to `/login` — which a browser blocking the cross-site `SameSite=None` session cookie will cause on the preview deployment — would auto-sign-in and re-assign forever. The claim bounds **that leg only**: a preview session whose cookie never lands is still unusable, because `/statistics` finds no session and the root route sends it back to `/login`. That inner bounce predates the continuation work and is not what the claim addresses.
