...
同じ再取得先の問題はcurrency取引の3操作にもあったため、通貨切替中の完了を再現する3ケースをred確認してから修正した。切替元のcacheをstaleにし、切替先への不要なrefetchと表示変更を防ぐ。今回のscoped実行はhelper関連56ケース、実useSessions 72ケース、実useCurrencies 52ケースの計180ケースが成功した。型・lint・規約検査も成功し、全体の検証は最新HEADのCIで確認する。
