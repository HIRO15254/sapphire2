...
function clientRow(overrides: Partial<ReviewRow> = {}): ReviewRow {
	return {
		rowId: "seat-1",
		action: "new",
		ambiguous: false,
...
describe("applyRow", () => {
	...
	it.each([
		[
			"hero",
			clientRow({ action: "hero", seatPosition: 4 }),
			trpcClient.liveCashGameSession.updateHeroSeat.mutate,
		...
		[
			"new",
			clientRow({ action: "new", name: "  Bob  ", seatPosition: 5 }),
	...
	it("resolves false when the mutate call rejects", async () => {
		...
		const result = await applyRow(
			clientRow({ action: "new", name: "Carol", seatPosition: 1 }),
			cashSession
		);
