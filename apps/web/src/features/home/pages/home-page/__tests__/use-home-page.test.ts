...
import { useHomePage } from "@/features/home/pages/home-page/use-home-page";
...
describe("useHomePage", () => {
	...
	describe("statusText", () => {
		it("is 'Checking...' while loading", () => {
			mocks.healthCheck.isLoading = true;
			const { result } = renderHook(() => useHomePage());
			expect(result.current.statusText).toBe("Checking...");
		});

		it("is 'Connected' when not loading and connected", () => {
			mocks.healthCheck.data = { ok: true };
			const { result } = renderHook(() => useHomePage());
			expect(result.current.statusText).toBe("Connected");
		});

		it("is 'Disconnected' when not loading and not connected", () => {
			mocks.healthCheck.data = null;
			const { result } = renderHook(() => useHomePage());
	...
	describe("statusDescription", () => {
		it("describes the pending check while loading", () => {
			mocks.healthCheck.isLoading = true;
			const { result } = renderHook(() => useHomePage());
			expect(result.current.statusDescription).toBe(
				"Checking server connectivity before you continue."
			);
		});

		it("reports readiness when connected", () => {
			mocks.healthCheck.data = { ok: true };
			const { result } = renderHook(() => useHomePage());
			expect(result.current.statusDescription).toBe("Everything looks ready.");
		});

		it("reports unavailability when disconnected", () => {
			mocks.healthCheck.data = null;
			const { result } = renderHook(() => useHomePage());
			expect(result.current.statusDescription).toBe(
				"Server connection is unavailable right now."
			);
		});
	});
	...
	describe("combined state", () => {
		it("reports connected + signed in with user name", () => {
			...
			expect(result.current).toEqual({
				...
				statusDescription: "Everything looks ready.",
			});
		});
	});
});
