...
describe("aggregatePnlPoints", () => {
	describe("xAxis = 'date'", () => {
		...
		it("sorts unordered input by sessionDate before bucketing", () => {
			const day1 = Math.floor(
			...
			const result = aggregatePnlPoints({
				...baseOptions,
				rawPoints: [
					point({ id: "b", profitLoss: 50, sessionDate: day2 }),
					point({ id: "a", profitLoss: 100, sessionDate: day1 }),
				],
			});
	...
	describe("unit = 'normalized', sessionType = 'cash_game'", () => {
		...
		it("skips cash rows with a non-positive bigBlind (cannot normalize)", () => {
			const result = aggregatePnlPoints({
				...baseOptions,
				xAxis: "sessionCount",
				unit: "normalized",
				sessionType: "cash_game",
				rawPoints: [
					point({ id: "a", profitLoss: 200, sessionDate: 1, bigBlind: 0 }),
	...
	describe("unit = 'normalized', sessionType = 'tournament'", () => {
		...
		it("skips tournament rows with a non-positive buyInTotal", () => {
			const result = aggregatePnlPoints({
				...baseOptions,
				xAxis: "sessionCount",
				unit: "normalized",
