...
- **No self check-in after opening a PR.** Do not use `send_later` / `create_trigger` to schedule a self check-in that re-checks a newly opened PR later — react to `<github-webhook-activity>` events (or ask the user) instead; scheduled self-reminders are unnecessary noise on routine PRs in this repo.
