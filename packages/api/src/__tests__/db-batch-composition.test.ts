...
describe("persistSessionChipPurchases batch composition (SA2-116)", () => {
...
describe("persistSessionBlindLevels batch composition (SA2-116)", () => {
...
describe("snapshotTournamentStructure batch composition (SA2-116)", () => {
...
describe("resnapshotTournamentStructure batch composition (SA2-116)", () => {
...
describe("persistCashSessionReopenEvents batch composition (SA2-116)", () => {
...
describe("session.create batch composition (SA2-116)", () => {
	it("submits gameSession + cash detail (+ auto ring game) in a single batch", async () => {
		const { db, batchCalls } = createBatchTrackingDb();
	...
	it("submits gameSession + tournament detail + copied structure in the same batch", async () => {
...
describe("tournament.createWithLevels batch composition (SA2-116)", () => {
	it("submits the tournament and all tags / chip purchases / blind levels in one batch", async () => {
		const rows = new Map<unknown, Rows>([
			[room, [{ id: "room-1", userId: "user-1" }]],
	...
	it("submits a bare tournament (no children) as a single-statement batch", async () => {
		const rows = new Map<unknown, Rows>([
			[room, [{ id: "room-1", userId: "user-1" }]],
			[tournament, [{ id: "tn-created", roomId: "room-1", name: "T" }]],
		]);
...
describe("live session create batch composition", () => {
	it("submits a cash session, frozen detail, and start event in one batch", async () => {
		const { db, batchCalls } = createBatchTrackingDb();
	...
	it("propagates a cash create batch failure after submitting parent, detail and event together", async () => {
		const { db, batchCalls } = createBatchTrackingDb(new Map(), {
	...
	it("submits a tournament session, frozen detail, and start event in one batch", async () => {
		const { db, batchCalls } = createBatchTrackingDb();
	...
	it("propagates a tournament create batch failure after submitting parent, detail and event together", async () => {
		const { db, batchCalls } = createBatchTrackingDb(new Map(), {
			batchError: new Error("D1 batch failed"),
		});
...
describe("live master create-and-assign batch composition", () => {
	it("submits a new ring game, session patch, and frozen cash snapshot in one batch", async () => {
		const rows = new Map<unknown, Rows>([
			[
				gameSession,
				[
	...
	it("propagates ring-game assignment batch failure after submitting both writes together", async () => {
		const rows = new Map<unknown, Rows>([
			[
				gameSession,
				[
	...
	it("submits a new tournament, all master children, assignment, and full frozen structure in one batch", async () => {
		const rows = new Map<unknown, Rows>([
			[
				gameSession,
				[
	...
	it("propagates tournament assignment batch failure after submitting both writes together", async () => {
		const rows = new Map<unknown, Rows>([
			[
				gameSession,
				[
	...
	it("in the same batch clears the old frozen structure when the newly created tournament has no children", async () => {
		const rows = new Map<unknown, Rows>([
			[
				gameSession,
				[
...
describe("tournament.updateWithLevels batch composition (SA2-116)", () => {
	it("submits the tournament UPDATE and every clear-and-reseed group in one batch", async () => {
		const rows = new Map<unknown, Rows>([
			[
				tournament,
...
describe("tag deletion batch composition (SA2-116)", () => {
	...
	it("propagates player-tag deletion batch failure without a second batch", async () => {
		const rows = new Map<unknown, Rows>([
			[playerTag, [{ id: "pt-1", userId: "user-1" }]],
		]);
		const { db, batchCalls } = createBatchTrackingDb(rows, {
	...
	it("propagates session-tag deletion batch failure without a second batch", async () => {
		const rows = new Map<unknown, Rows>([
			[sessionTag, [{ id: "st-1", userId: "user-1" }]],
		]);
		const { db, batchCalls } = createBatchTrackingDb(rows, {
