...
export const TOOL_DEFINITIONS: ToolDefinition[] = [
	...
	},
	{
		...
		description: `Aggregate statistics (profit/loss, win rate, session counts, EV metrics) over the user's sessions. currencyId is required unless normalized is true. normalized does NOT convert or restate anything — it only lifts that requirement, so the query then spans every currency. The response is the same either way and always carries both kinds of figure. Anything built from raw amounts is meaningless once more than one currency is in scope: totalProfitLoss, avgProfitLoss, hourlyRate, totalPrizeMoney, totalEvProfitLoss, totalEvDiff, and also roi — a percentage, but one raw cross-currency sum divided by another. The currency-safe figures are cashNormalizedProfitLoss (big blinds, over cashBbCount), tournamentNormalizedProfitLoss (buy-ins, over tournamentBiCount), bbPerHour, cashEvDiffNormalized and avgRoi (the mean of per-session ROI, unlike roi). bb and bi are different scales that must never be summed. Pass a currencyId whenever the user asked about money. Optional filters: type, roomId, dateFrom/dateTo. ${DATE_CONVENTIONS}`,
