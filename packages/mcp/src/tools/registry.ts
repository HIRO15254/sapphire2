...
export const TOOL_DEFINITIONS: ToolDefinition[] = [
	...
	},
	{
		...
		description: `Update a ring-game rule master by id. Only the supplied fields change; pass null to clear a nullable one. ${MIX_RULE} To edit only the blinds of a mixed rule, echo back the mixGames that ring_game_list_by_room returned. Two kinds of rule never come back from that list even though this tool still updates them: an archived one (the list is either/or on archive state — pass includeArchived for those) and one auto-created by session_create_cash_game without a ringGameId, which is stored with no roomId and so matches no room at all. For the latter, take ringGameId and cashMixGames off the session itself via session_get_by_id — both were frozen from the same snapshot as the rule, so cashMixGames is exactly what to echo — but only while the mix master is untouched, because the check rebuilds the expected shape from the CURRENT variant labels and groups and the mix's current games. After a game_mix_update that replaced games, a game_variant_update that changed a label or group, or a game_group_update that renamed a user-created group (those sort by label, and that order is the entry order), rebuild mixGames from the current masters instead. Renaming the mix inverts that: variant is a copied label rather than a foreign key, so it stops resolving and the stored mixGames becomes the reference instead of the masters — send its grouping and variant labels back unchanged (only the per-entry blinds may differ), or set variant to the new mix label and send a rebuilt mixGames. To move a mixed rule back to flat blinds, set variant to a non-mixed label — that clears mixGames for you. Sending mixGames: null on its own is rejected either way, because leaving variant unchanged keeps the rule mixed. ${AMOUNT_CONVENTIONS}`,
