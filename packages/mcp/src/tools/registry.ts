...
export const MIX_RULE =
	'A mixed-game rule is variant + mixGames together: variant must be the label of a game mix from game_mix_list (or the legacy "mix" sentinel), and mixGames its rotation. Sending mixGames without such a variant is rejected. For a named mix, mixGames must reproduce that mix EXACTLY: one entry per game group its variants belong to, entries in the order game_group_list returns those groups, and each entry naming its variants by their game_variant_list label in the mix\'s own games order. Note game_mix_list returns variant IDS, not labels, so build the labels from game_variant_list. Any other grouping or order is rejected as "references an unavailable game master" — the message names ownership, but the cause is usually shape. The legacy "mix" sentinel is the loose form: any owned variant labels, grouped and ordered however you like, except that the variants of each entry must belong to ONE game group (their groupId in game_variant_list) — an entry spanning two groups is rejected with "must share a game group". An entry already stored that way (a variant moved group after it was saved), on the record being updated or on the ring game passed as ringGameId in the same call, is still accepted when sent back unchanged or with games removed, but no game can be added to it. While a mix is set, the top-level blind1-3, ante and anteType are always stored as null, so values sent for those flat fields are dropped — the blinds of a mix live on each mixGames entry instead (its own blind1-3 / ante / anteType, per group).';

export const LEVEL_GAMES_RULE =
	'A blind level may carry games: groups of variant labels, each with its own blind1-3 / ante, played in rotation during that level. The variants of each group must belong to ONE game group (their groupId in game_variant_list) — a group spanning two is rejected with "must share a game group". A group already stored that way (a variant moved group after it was saved), on the record being updated or on the tournament master passed as tournamentId in the same call, is still accepted when sent back unchanged or with games removed, but no game can be added to it.';

export const TOOL_DEFINITIONS: ToolDefinition[] = [
	{
		name: "session_list",
		procedurePath: "session.list",
		description: `List the user's completed poker sessions, newest first, 20 per page. Optional filters: type ("cash_game" | "tournament"), roomId, currencyId, dateFrom/dateTo (inclusive). Returns { items, nextCursor, summary }; summary aggregates ALL matching sessions, not only the returned page. Its totalEvProfitLoss / totalEvDiff are cash-only (so they trail totalProfitLoss by the tournaments) and are null unless at least one cash session in scope recorded an EV cash-out; once one did, they span every finished cash session, counting the ones that recorded no EV at their actual result, which adds 0 to the diff — so a 0 diff means the recorded EV matched the results, not that EV is untracked. Same rule as stats_summary, over the same scope. Pass nextCursor back as cursor for the next page. ${DATE_CONVENTIONS}`,
	...
	},
	{
		...
		description: `Aggregate statistics (profit/loss, win rate, session counts, EV metrics) over the user's sessions. currencyId is required unless normalized is true. normalized does NOT convert or restate anything — it only lifts that requirement, so the query then spans every currency. The response is the same either way and always carries both kinds of figure. Anything built from raw amounts is meaningless once more than one currency is in scope: totalProfitLoss, avgProfitLoss, hourlyRate, totalPrizeMoney, totalEvProfitLoss, totalEvDiff, and also roi — a percentage, but one raw cross-currency sum divided by another. The currency-safe figures include cashNormalizedProfitLoss (big blinds, over cashBbCount), tournamentNormalizedProfitLoss (buy-ins, over tournamentBiCount), bbPerHour, cashEvDiffNormalized and avgRoi (the mean of per-session ROI, unlike roi); the counts and rates (totalSessions, winRate, itmRate, avgPlacement, totalPlayMinutes) do not depend on currency at all — winRate just counts profitLoss > 0. The two normalized P/L figures cover only the sessions that have a denominator: totalSessions minus (cashBbCount + tournamentBiCount) is exactly how many were left out — a cash session with no big blind, which every mixed-game rule produces since those store blind1-3 as null, and a tournament with nothing invested — while those sessions still count in totalProfitLoss and winRate. cashEvDiffNormalized covers the cash sessions that have a big blind AND a settled result, so it is narrower than cashNormalizedProfitLoss by exactly the sessions still in progress, and its count is not returned, and bbPerHour divides the same bb subtotal by ALL cash hours, so it is pulled toward zero whenever some cash sessions have no big blind. The three EV figures (totalEvProfitLoss, totalEvDiff, cashEvDiffNormalized) are null unless a session each one actually covers has a recorded EV cash-out — for the two raw figures that is any cash session in scope, for cashEvDiffNormalized it has to be one with a big blind. Once one does, they span EVERY finished cash session they cover, not an EV-only subset: a session with no recorded EV cash-out is treated as having run exactly at EV, so it adds its actual result to totalEvProfitLoss and 0 to the two diffs. So totalEvProfitLoss is comparable with the CASH part of totalProfitLoss — that one also carries the tournaments, so the two match only in a cash-scoped query — and totalEvDiff being 0 means the recorded EV matched the results, NOT that EV is untracked; untracked reads as null. bb and bi are different scales that must never be summed. Pass a currencyId whenever the user asked about money. Optional filters: type, roomId, dateFrom/dateTo. ${DATE_CONVENTIONS}`,
	...
	},
	{
		...
		description: `Create a tournament master inside a room, together with its blind levels, chip purchases and tags in one call. Required: roomId, name. blindLevels are ordered as given (level 1 first); every row requires isBreak (true for a break, false for a playing level). ${LEVEL_GAMES_RULE} ${AMOUNT_CONVENTIONS}`,
	...
	},
	{
		...
		description: `Update a tournament master by id. blindLevels is REQUIRED and REPLACES the entire structure — read tournament_get_by_id first and send back the full list, or the existing levels are lost. tags and chipPurchases replace their lists too when supplied. ${LEVEL_GAMES_RULE} ${AMOUNT_CONVENTIONS}`,
